...
syntax varlist [if] [in] [aweight pweight fweight iweight] [, DELta(real 1) k(real 1) excluded(varlist) NONparametric LIMit(real 1e-8) from(name)  u_hat(varlist)  MAXimum(real 10000) Robust CLuster(string) ABSorb(string)]   
	marksample touse
	local list_var `varlist'
	* get depvar and indepvar
	gettoken depvar list_var : list_var
	gettoken indepvar list_var : list_var, p("(")
   iOLS_MP_HDFE `varlist' `excluded' if `touse' , delta(`delta') limit(`limit') from(`from') maximum(`maximum') 
  quietly: replace `touse' = e(sample)
	matrix beta_hat = e(b)
	matrix var_cov_beta_hat = e(V)
	* rhs of test
	tempvar E_u_hat
    quietly: egen `E_u_hat' = mean(iOLS_MP_HDFE_error) if `touse'
    tempvar lhs_temp
	quietly: gen `lhs_temp' = iOLS_MP_HDFE_error
tempvar dep_pos

cap program drop i2SLS_MP_HDFE_test
program define i2SLS_MP_HDFE_test, eclass 
syntax varlist [if] [in] [aweight pweight fweight iweight] [, DELta(real 1) k(real 1) endog(string) instr(string)  LOGIT NONparametric LIMit(real 1e-3) from(name) MAXimum(real 10000) absorb(string)] 
	marksample touse
	local list_var `varlist'
	* get depvar and indepvar
	gettoken depvar list_var : list_var
	gettoken indepvar list_var : list_var, p("(")
 quietly:  i2SLS_MP_HDFE `varlist' if `touse' ,   endog(`endog') instr(`instr') absorb(`absorb') delta(`delta') limit(`limit') from(`from') maximum(`maximum') 
...
di in red "Using Logit Probability Model"
		...
		quietly:	xi: logit `dep_pos' `instr' `indepvar' `vlist1' if `touse'
		tempvar p_hat_temp
...
di in red "Using Linear Probability Model"
		quietly: reghdfe `dep_pos' `instr'  `indepvar'  if `touse' , absorb(`absorb') resid 
		tempvar p_hat_temp
 ...
 if  "`nonparametric'" =="" {
di in red "Using Logit Probability Model"
quietly: logit `dep_pos' `instr' `indepvar' if `touse'
tempvar p_hat_temp
quietly:predict `p_hat_temp' if `touse', pr 
cap drop lambda_stat
quietly: gen lambda_stat = 1/`p_hat_temp' if `touse'
...
local k = floor(sqrt(r(N))) 
 ...
 quietly: _rmcoll `indepvar' `instr' if `touse', forcedrop 
